...
public class Main
{
	...
	public static void main(String[] args)
	{
		...
		succes = true;
			...
			} while(out.equals("yes") || out.equals("no"));
	...
	public static void tippResult()
	{
		...
		for (int i = 0; i < 4; i++)
		{
			for (int k = i+1; k < 4; k++)
			{
				if (tippColors.get(i) == goodColors.get(k) || goodColors.get(i) != null)
				{
					result += "| ";
					goodColors.set(i, null);
				}
	...
	public static void resultProcessing()
	{
		if (result.equals("X X X X "))
		{
			...
			{
				System.out.println("Your " + chances+1 + "th tipp's result: " + result);
			}
			else
			{
				succes = false;
